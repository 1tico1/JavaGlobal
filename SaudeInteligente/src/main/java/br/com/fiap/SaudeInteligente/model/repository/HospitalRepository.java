...
import br.com.fiap.SaudeInteligente.model.entity.Hospital;
...
public class HospitalRepository extends Repository {
	    ...
	    public static Hospital save(Hospital hospital) {
	        String sql = "insert into tb_hospital"
	        ...
	        public static boolean delete(Long id) {
	            String sql = "delete from tb_hospital where id = ?";
	            try (PreparedStatement ps = getConnection().prepareStatement(sql)) {
	                ps.setLong(1, id);
		...
		public static List<Hospital> findByNome(String nome) {
	        ...
	        String sql = "SELECT * FROM tb_hospital WHERE nome LIKE ?";
	        try (PreparedStatement ps = getConnection().prepareStatement(sql)) {
	            ps.setString(1, "%" + nome + "%");
